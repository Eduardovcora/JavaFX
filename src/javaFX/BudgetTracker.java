...
public class BudgetTracker extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        trans.setPromptText("Enter the amount");

        TextField title = new TextField();
        title.setPromptText("Type your title");

        TextField category = new TextField();
        category.setPromptText("Type your category");
        ...
        sort.setOnAction(action -> {
            ...
            for (int i = 0; i < core.getTime().size(); i++) pos.add(i);

            pos.sort(Comparator.comparing(core.getTime()::get));
    ...
    private String formatTransaction(int i) {
        double amount = core.getTransactions().get(i);
        String type = amount >= 0 ? "Income" : "Expense";
        return "Transaction #" + i + " | Type: " + type + " | Title: " + core.getTitles().get(i) + " | Amount: " + amount + " | Category: " + core.getCategories().get(i) + " | Date & Time: " + core.getTime().get(i).format(formatter);
    }
